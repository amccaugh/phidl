...
def test_comb(pad_size = (200,200), wire_width = 1, wire_gap = 3,
    ...
    while(obj.xmax + pad_size[0]/12 < padr_nub.xmin):
        ...
        if(top):
            ...
            top = True
            # comb rectange
            comb = CI.add_ref(rectangle(size=((padt.ymin-head.ymax)+pad_size[1]/2  - (wire_gap + wire_width)/2, wire_width), layer=comb_layer))
            comb.rotate(90)
            comb.ymax = padt.ymin
...
def test_ic(wire_widths = [0.25, 0.5,1,2,4], wire_widths_wide = [0.75, 1.5, 3, 4, 6], pad_size = (200,200),
            wire_layer = 0, pad_layer = 1, gnd_layer = None, pad_gap=75):
    """
    Usage:
