...
def ccRings(radius = 10, gaps = [0.1, 0.2, 0.3], wRing = 0.5, wWg = 0.4, dR = 0.15, period = 30):
    nrings = len(gaps)
    lWg = (nrings + 1)*period
    ...
    for i in range(nperiods):
        cgrating = G.add_ref(pg.compass(size=[period*ff,wGrating]))
        cgrating.x+=i*period
    ...
    pg.route(port1 = g.ports[1], port2 = wg.ports['W'])
    pg.route(port1 = g2.ports[1], port2 = wg.ports['E'])
