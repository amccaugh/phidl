...
#            return [self.add_polygon(p, layer) for p, layer in zip(points.polygons, layers)]
    ...
    def remap_layers(self, layermap = {}, include_labels = None, recursive = True):
        if include_labels is not None:
            warnings.warn('[PHIDL] remap_layers(): The `include_labels` argument is' + 
                ' deprecated and no longer functions, try moving your labels to a ' +
                ' different layer or datatype')
    ...
    def remove_layers(self, layers = (), invert_selection = False, include_labels = None, recursive = True):
        if include_labels is not None:
