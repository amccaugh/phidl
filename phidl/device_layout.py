...
class Device(gdspy.Cell, _GeometryHelper):
    ...
    def label(self, text = 'hello', position = (0,0), magnification = None, layer = 255):
        if len(text) >= 1023:
            raise ValueError('[DEVICE] label() error: Text too long (limit 1024 chars)')
        gds_layer, gds_datatype = _parse_layer(layer)

        if type(text) is not str: text = str(text)
        self.add(gdspy.Label(text = text, position = position, anchor = 'o', magnification = magnification,
